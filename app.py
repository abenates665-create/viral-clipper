...
# Read Gemini API key from environment (set on Render)
GEMINI_API_KEY = os.environ.get('GEMINI_API_KEY',AIzaSyAVcos3bfx1AHc4HnindaANPLgClkalvHM '')
...
def process_video(job_id, url, preset, clip_count=1):
    ...
    try:
        ...
        clips_generated = []
        for i in range(min(clip_count, 20)):
            start_time = i * clip_duration
            if start_time >= duration:
                break
            output_file = f"outputs/{job_id}_clip_{i+1}.mp4"
            ff_cmd = f"-i {download_path} -ss {start_time} -t {clip_duration} -vf 'scale=1080:1920:force_original_aspect_ratio=decrease,pad=1080:1920:(ow-iw)/2:(oh-ih)/2' -c:a aac -y {output_file}"
            run_ffmpeg(ff_cmd)
            clips_generated.append(f"{job_id}_clip_{i+1}.mp4")
            job.progress = int(80 * (i + 1) / clip_count)
            db.session.commit()
    except Exception as e:
    ...
    try:
        ...
        if json_match:
            ...
            metadata = {"titles": ["Viral Clip!", "Watch This!", "Insane Moment", "Must See!", "Trending Now"], "description": "Auto-generated clip."}
    except Exception as e:
        metadata = {"titles": ["Clipped Video", "Short Form", "Trending", "Viral Moment", "Watch Now"], "description": "Auto-generated."}
...
def start_process():
    ...
    clip_count = data.get('clips', 1)
    job_id = str(uuid.uuid4())[:8]
    new_job = Job(id=job_id, url=url, preset=preset, status='queued')
    db.session.add(new_job)
    db.session.commit()
    thread = threading.Thread(target=process_video, args=(job_id, url, preset, clip_count))
    thread.start()
...
def schedule_job():
    ...
    scheduler.add_job(func=process_video, trigger='date', run_date=run_time, args=[job_id, url, preset, clip_count])
    scheduler.start()
